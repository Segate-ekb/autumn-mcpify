...
> Ни один класс библиотеки не делает `#Использовать autumn-mcp` — бин `MCP_Сервер`
> резолвится в рантайме по имени. Поэтому загрузка `autumn-mcpify` в обычной точке
> входа CLI не тянет рогатку autumn-mcp и не ломает штатный запуск приложения.
